...
contract MetaPEPE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MetaPEPEDividendTracker public dividendTracker;
    ...
    uint256 public maxTransactionAmount = 2 * (10**12) * (10**18); //2%
    ...
    uint256 public maxWalletToken = 2 * (10**12) * (10**18); //2%

    address public _marketingWalletAddress = 0x49a52804Af42Ab68e13F6019f4c149a68D7B40D4;
    address public _DevWalletAddress = 0x63312ad4Eb2e57C5a90Bacab3484F2553495d010;
    ...
    constructor() ERC20("MetaPepe", "MetaPEPE") {

    	dividendTracker = new MetaPEPEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MetaPEPE: The dividend tracker already has that address");

        MetaPEPEDividendTracker newDividendTracker = MetaPEPEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MetaPEPE: The new dividend tracker must be owned by the MetaPEPE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MetaPEPE: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MetaPEPE: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "MetaPEPE: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "MetaPEPE: Account is already the value of 'excluded'");
    ...
    function setMetaPEPEWallet(address payable wallet) external onlyOwner{
         require(wallet != address(0), "Invalid address");
    ...
    function setSellFees(uint256 sellEthRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 SellDevFee) external onlyOwner
    {
      ...
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MetaPEPE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "MetaPEPE: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "MetaPEPE: Cannot update gasForProcessing to same value");
...
contract MetaPEPEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MetaPEPE_Dividend_Tracker", "MetaPEPE_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 50000000 * (10**18); //must hold 50000000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MetaPEPE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MetaPEPE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaPEPE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MetaPEPE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MetaPEPE_Dividend_Tracker: Cannot update claimWait to same value");
